...
console.log('\nbuildInstallPacing');
{
  ...
  const conv = [
    ...
    ca('2026-09-10', 'Inapp - GA4 (web) app_install_attributed', 3), // camp đích website → đếm
    ca('2026-09-10', 'Inapp (Upload) app_install_attributed (1)', 2), // bản upload trùng → không đếm
    ca('2026-09-03', 'Inapp (Upload) app_install_attributed (1)', 2), // bản upload trùng → không đếm
  ];
  const p = buildInstallPacing(shop, conv, 7);
  eq('kỳ theo ngày cuối export Shopify', [p.from, p.to], ['2026-09-08', '2026-09-14']);
  eq('Shopify 14 + Google 3+3; click_get_app và bản Upload (1) không tính', [p.shopifyInstalls, p.googleInstalls, p.installs], [14, 6, 20]);
  ...
  near('pct = installs ÷ target', p.pct, 20 / ((sept / 30) * 7));
