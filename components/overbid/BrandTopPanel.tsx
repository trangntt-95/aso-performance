          ...
          <table className="w-full text-xs">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="px-2 py-1.5 text-right font-medium" title="Bid hiện tại = median 'Bid (max)' của keyword trong camp (Master KW Lookup) · Bid rec ⭐ = trung bình ô Brand × nước target trong 'Max bid cap'">Bid nay / rec ⭐</th>
...
function BrandRow({ r }: { r: BrandTopRow }) {
  ...
  return (
    <tr className={cn('border-t align-top hover:bg-slate-50', r.verdict === 'top' && 'bg-amber-50/40')}>
      ...
      <td className="px-2 py-1.5 text-right whitespace-nowrap font-mono text-[11px]">
        <span className={cn(overRec ? 'font-semibold text-rose-600' : 'text-slate-800')} title={overRec ? 'Bid hiện tại cao hơn Bid rec ⭐ trên 15%' : undefined}>
          {money(r.bidNow)}
        </span>
        <span className="text-slate-400"> / {money(r.bidRec)}</span>
      </td>
      <td className="px-2 py-1.5 text-right whitespace-nowrap font-mono text-[11px]">
        {r.organicPos === null ? (
