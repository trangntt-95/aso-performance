      ...
      {open && (
        <tr className="bg-slate-50/60">
          <td colSpan={7} className="px-2 py-2">
            {r.sliceMembers.length > 0 && (
              <>
                <div className="pl-5 text-[10px] uppercase tracking-wide text-slate-500">
                  {r.category} gồm
                </div>
                <ul className="mt-1 mb-2 space-y-0.5 pl-5">
                  {r.sliceMembers.map((m) => (
                    <li key={m.label} className="flex items-baseline gap-2 text-[11px]">
                      <span className="min-w-0 flex-1 truncate font-medium text-slate-700">
                        {m.label}
                      </span>
                      <span className="shrink-0 font-mono text-slate-600">{money(m.spend)}</span>
