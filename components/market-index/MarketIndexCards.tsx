...
export function MarketIndexCards() {
  ...
  return (
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      {/* Nửa trên hết ở đây. Mọi số phía trên là toàn thị trường, không cân —
          "thị trường đang lên hay xuống" phải là câu hỏi về cả thị trường, cân
          theo tiền ở đó sẽ thành câu hỏi khác mà vẫn mang tên cũ.

          Từ đây xuống mới là câu hỏi "nước nào đáng nặng", nên trọng số sống ở
          phần dưới và có công tắc riêng. */}
      {!isLoading && market?.basket && market.basket.length > 0 && (
        <DynamicBasketCard data={market.basket} />
      )}

      {!isLoading && <CountryWeightSection data={data} />}
    </div>
  );
}
